...
## 最近更新 (v3.31.0 ~ v3.32.0)

### v3.32.0 — `doc_indent_block` 區塊縮排工具

支援文件區塊縮排/取消縮排。Lark 透過 parent-child block 關係實現縮排，此工具透過 delete + re-insert 遞迴搬移整棵 block tree。

- `indent`：將 block 移到前一個 sibling 底下（成為其子節點）
- `outdent`：將 block 提升到 grandparent 層級（從巢狀中提出）
- 支援含子節點的 block，整棵子樹一併搬移

### v3.31.0 — RateLimiter queue-based 修復

修復 `tasklist_tasks` 大量任務時約 30% 回傳 `(failed to fetch)` 的問題。根因是 `Promise.all()` 並發呼叫時，timestamp-based throttle 無法正確序列化請求，導致同時送出大量請求觸發 Lark 3 QPS 限流。

- `RateLimiter` 從 timestamp-based 改為 promise-chain-based，並發呼叫自動排隊依序執行
- `RATE_LIMIT_INTERVAL_MS` 350 → 340ms（start-to-start 間隔，≈2.94 QPS）
