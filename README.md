...
> 使用 `section_list` + `section_tasks` 可取得「我負責的」中的所有未完成任務。
...
| response_format | string | "json" | 輸出格式："json"（原始資料）或 "markdown"（人類可讀）|

> **說明**：預設使用 JSON 格式讓 AI 處理原始資料，只在需要顯示給使用者時才使用 Markdown。
