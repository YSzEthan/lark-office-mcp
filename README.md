...
## 最近更新 (v3.33.0 ~ v3.34.0)

### v3.34.0 — 容器 block `_children` 支援

寫入工具新增 `_children` 欄位，支援 Callout、Quote 等容器 block 自動建立父 block 並遞迴插入子 block。

- 容器 block 使用 `_children` 陣列指定子 block，支援多層嵌套
- `insertBlocks` 遇到 `_children` 時自動 flush → 建立父 block → 遞迴插入
- `hasTable` 重新命名為 `hasNestedBlocks`，正確反映「需要多步驟插入的 block」語意

### v3.33.0 — `tasklist_tasks` 新增 `completed` 過濾

`tasklist_tasks` 新增 `completed` (boolean, optional) 參數，可依完成狀態過濾任務，與 `section_tasks` 行為一致。
